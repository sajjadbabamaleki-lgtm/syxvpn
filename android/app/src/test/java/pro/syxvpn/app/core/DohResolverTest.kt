...
class DohResolverTest {
    ...
    private fun answer(vararg addresses: String, ttl: Int = 300): String {
        val records = addresses.joinToString(",") { address ->
            "{\"name\":\"control.syxvpn.pro\",\"type\":1,\"TTL\":$ttl,\"data\":\"$address\"}"
        }
    ...
    fun `reads the addresses out of an answer`() {
        ...
        assertEquals(listOf("173.249.47.5"), resolver.addresses("control.syxvpn.pro"))
    }
    ...
    fun `keeps only A records, because a CNAME's data is a name`() {
        val mixed = """
            {"Status":0,"Answer":[
              {"name":"control.syxvpn.pro","type":5,"TTL":300,"data":"edge.example.net"},
    ...
    fun `an answer with no addresses is not an answer`() {
        ...
        assertTrue(resolver.addresses("control.syxvpn.pro").isEmpty())
    }
    ...
    fun `moves to the second operator when the first fails`() {
        ...
        assertEquals(listOf("10.1.2.3"), resolver.addresses("control.syxvpn.pro"))
    ...
    fun `does not ask again while the answer is still good`() {
        ...
        resolver.addresses("control.syxvpn.pro")
        resolver.addresses("control.syxvpn.pro")
    ...
    fun `forgetting an address makes the next call ask`() {
        ...
        resolver.addresses("control.syxvpn.pro")
        resolver.forget("control.syxvpn.pro")
        resolver.addresses("control.syxvpn.pro")
        assertEquals(2, calls)
    }
    ...
    fun `the host being asked about is the host in the query`() {
        ...
        resolver.addresses("gw1.syxvpn.pro")
        assertTrue("query carries the name: $asked", asked!!.endsWith("name=gw1.syxvpn.pro"))
    }
