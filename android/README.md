...
The workflow therefore caches `~/.android/debug.keystore` under a fixed key, and
prints the APK's certificate digest so the identity can be checked rather than
assumed. GitHub drops a cache that goes unused for a week; when that happens the
next build starts a new identity, and that one APK needs the old app removed
first.
